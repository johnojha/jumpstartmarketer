...
<cfparam name="url.slider" default=0>
...
<cfparam name="url.fid" default=0>
	...
	<cfquery name="s" datasource="#application.dsn#"> 
		SELECT c.name, c.email, b.business_desc, c.business_type, b.benefit
	...
	<!--- new code by john for checking consumer not to add more than 5 feedback --->
<cfquery name="consumer_#trim(choices.business_element)#" datasource="#application.dsn#">
			SELECT DISTINCT UUID
			FROM  t_ratings
			WHERE     (cid = #client.cid#)
				AND business_element = '#trim(choices.business_element)#'
		</cfquery>
<cfset conss=evaluate('consumer_' & trim(choices.business_element) & '.recordcount')>

<cfoutput>#conss#</cfoutput>
<cfif conss gte 5>

<cfoutput>Your Session Has Been Expired . More than 5 people have already responded</cfoutput>
...
<!--- End new code by john for checking consumer not to add more than 5 feedback --->
	...
	<cfoutput>
		...
		<form name="Feedback" action="act_feedback.cfm?primary=#url.primary#&rid=#url.rid#&fid=#url.fid#&uuid=#url.uuid#" method="post">
    </cfoutput>
			<cfoutput query="s">
				<tr>		  
				...
				<tr>		  
					<td valign="top" colspan="2">
						<font color="red">NOTE:</font> When rating, please score on the high side or the low side,  we prefer you stay away from rating the options in the middle of the spectrum.	
					</td>			
				</tr>
			</cfoutput>		
			...
			<cfif choices.business_element eq "brand" or choices.business_element eq "sub_brand">	
				...
				<cfelseif choices.business_element eq "brand_logo">

					<br><br>
					JUDGING CRITERIA:<br>
						<ol>
				<li>Must represent the BRAND in a remarkable way.</li>

				 </ol>
                 
				<cfelseif choices.business_element eq "sub_brand_logo">

					<br><br>
					JUDGING CRITERIA:<br>
						<ol>
				<li>Must represent the SUB-BRAND in a remarkable way.</li>
				...
				<li>Next, review the first Graphic Presentation image several times then review each of the three judging criteria again.</li>
				 	<br><br>
				<li>Use the sliders below to rate the Graphic Presentation from 1-100 (100 being most Remarkable). </li>
					<br><br>
				</ul>
				...
				<li>Next, review the <cfif choices.business_element neq "rtb">#ucase(replace(choices.business_element,"_"," ","All"))#<cfelse>ENDORSEMENT</cfif> several times.</li>
				 	<br><br>
				<li>Use the sliders below to rate the <cfif choices.business_element neq "rtb">#ucase(replace(choices.business_element,"_"," ","All"))#<cfelse>ENDORSEMENT</cfif> from 1-100 (100 being most Remarkable). </li>
					<br>
				</ul>
				</cfoutput>	
			</cfif>
			 ...
			 <tr>		  
				<td align="center" class="black10" colspan="2">
					...
					<cfelse>
						<font size="3"><cfoutput>#ucase(replace(choices.business_element,"_"," ","All"))#</cfoutput></font> 
					</cfif>
					that requires your feedback.
					<br>
					Move the sliders to rate each choice (there will be three), 100 being the best.
				</td>
			</tr>	
			...
			<tr>	
				<td align="center" colspan="2">	
					...
					<cfif choices.business_element eq "attributes">
						BENEFIT: <font size="+2">#s.benefit#</font>
						<br><br>
					</cfif>
  ...
  <cfif url.primary eq 1>                  
			...
			<tr>
				<td colspan="7" bgcolor="CCCCCC" align="center">
					<img src="images/#choices.business_element#/#choices.cid#/#choices.choice_1#">
						...
						<tr>
							<td width="75%" class="black14" <cfif choices.business_element eq "rtb">rowspan="2" bgcolor="FFFFCC"</cfif>>
								<cfif choices.business_element neq "rtb">
                                    1. <input type="text" name="choice_1" size="50" value="#choices.choice_1#" readonly="yes">
                                <cfelse>
                                	 #choices.choice_1#
									 <input type="hidden" name="choice_1" size="50" value="#choices.choice_1#" readonly="yes">
								</cfif> 
							</td>
      ...
      <!--- Show second choice to be rated.  --->             
				...
				<td colspan="7" bgcolor="CCCCCC" align="center">
					<img src="images/#choices.business_element#/#choices.cid#/#choices.choice_2#">
						...
						<tr>
							<td width="75%" class="black14" <cfif choices.business_element eq "rtb">rowspan="2" bgcolor="FFFFCC"</cfif>>
								<cfif choices.business_element neq "rtb">
                                    2. <input type="text" name="choice_2" size="50" value="#choices.choice_2#" readonly="yes">
                                <cfelse>
                                	 #choices.choice_2#
									 <input type="hidden" name="choice_2" size="50" value="#choices.choice_2#" readonly="yes">
								</cfif> 
							</td>
							<td width="250" background="images/spectrum.jpg" colspan="4">
								<div class="carpe_horizontal_slider_track">
				...
				<td colspan="7" bgcolor="CCCCCC" align="center">
					<img src="images/#choices.business_element#/#choices.cid#/#choices.choice_3#">
						...
						<tr>
							<td width="75%" class="black14" <cfif choices.business_element eq "rtb">rowspan="2" bgcolor="FFFFCC"</cfif>>
								<cfif choices.business_element neq "rtb">
                                    3. <input type="text" name="choice_3" size="50" value="#choices.choice_3#" readonly="yes">
                                <cfelse>
                                	 #choices.choice_3#
									 <input type="hidden" name="choice_3" size="50" value="#choices.choice_3#" readonly="yes">
								</cfif> 
							</td>
							<td width="250" background="images/spectrum.jpg" colspan="4">
								<div class="carpe_horizontal_slider_track">
